...
from foxblat.hid_handler import is_moza_device
...
class GenericSettings(SettingsPanel):
    def __init__(self, button_callback, settings: SettingsHandler):
        ...
        self._device_list: FoxblatPreferencesGroup = None
        self._candidates_list: FoxblatPreferencesGroup = None
        self._back: FoxblatPreferencesGroup = None
    ...
    def prepare_ui(self) -> None:
        ...
        self._current_group.set_present(False)
        self._add_row(FoxblatAdvanceRow("Go back"))
    ...
    def list_configured_devices(self, *_) -> None:
        ...
        if len(devices) < 1:
            row = FoxblatLabelRow("No configured devices")
            self._add_row(row)
            self._listed_devices["empty"] = row

        for device in devices:
            row = FoxblatButtonRow("", "Remove")
    ...
    def list_candidates(self, *_) -> None:
        ...
        for device in devices:
            if "foxblat" in device.name.lower():
                continue
            ...
            row = FoxblatButtonRow(device.name, "Fix Detection")
            row.subscribe(self.add_device, device.path, True)
            self._add_row(row)
            self._candidates.append(row)

        if len(self._candidates) < 1:
            row = FoxblatLabelRow("No promising devices")
            self._add_row(row)
            self._candidates.append(row)
    ...
    def _change_device_state(self, name: str, state: bool) -> None:
        ...
        GLib.idle_add(self._listed_devices[name].set_title, label)
