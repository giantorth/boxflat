...
from foxblat.hid_handler import HidHandler, MozaAxis
...
class SettingsPanel(EventDispatcher):
    def __init__(self, title: str, button_callback,
        ...
        self._current_group: FoxblatPreferencesGroup = None
        self._current_stack: Adw.ViewStack = None
        self._current_row: FoxblatRow = None

        self._groups: list[FoxblatPreferencesGroup] = []
    ...
    def set_banner_title(self, new_title: str):
        GLib.idle_add(self._banner.set_title, new_title)
    ...
    def active(self, value: int):
        ...
        for group in self._groups:
            GLib.idle_add(group.set_sensitive, value)
    ...
    def add_preferences_group(self, title="", level_bar=False, alt_level_bar=False, suffix=False):
        ...
        self._current_group = FoxblatPreferencesGroup(title, level_bar, alt_level_bar, suffix)
    ...
    def _add_row(self, row: FoxblatRow):
        if self._current_group is None:
            self.add_preferences_group()
        self._current_row = row

        if isinstance(row, FoxblatRow):
            row.set_width(620)
