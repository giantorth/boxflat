...
class HPatternSettings(SettingsPanel):
    def __init__(self, button_callback, connection_manager: MozaConnectionManager, settings: SettingsHandler, hid: HidHandler):
        ...
        self._gear_row = FoxblatLabelRow("Current Gear")
    ...
    def prepare_ui(self):
        ...
        row1 = FoxblatSliderRow("Auto Blip Output", 0, 100, suffix="% ", increment=10)
        row2 = FoxblatSliderRow("Auto Blip Duration", 0, 1000, subtitle="Miliseconds", increment=50)
        row1.set_active(0)
        row2.set_active(0)
        self._slider1 = row1
        self._slider2 = row2

        self._add_row(FoxblatSwitchRow("Auto Downshift Throttle Blip", subtitle="Easy rev match"))
        ...
        self._add_row(FoxblatCalibrationRow("Device Calibration", "Shift into R > 7th > R > Neutral"))
